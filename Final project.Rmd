...
p_mat <- matrix(NA, nrow = simulation, ncol = 2)
colnames(p_mat) <- c("X1", "X2")

select_mat <- matrix(0, nrow = simulation, ncol = p)
colnames(select_mat) <- paste0("X", 1:p)
...
for (i in 1:simulation) {
  ...
  if ("X1" %in% rownames(sm)) {
    p_mat[i, "X1"] <- sm["X1", "Pr(>|t|)"]
  }

  if ("X2" %in% rownames(sm)) {
    p_mat[i, "X2"] <- sm["X2", "Pr(>|t|)"]
  }
}
