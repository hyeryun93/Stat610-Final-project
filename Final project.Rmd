...
mean(rowSums(selected_matrix))
 
mean(typeI_error > 0)

 
mean(rowSums(selected_matrix[, null_vars]))

 
hist(rowSums(selected_matrix), main="Selected model size", xlab="Number of selected predictors")
abline(v=2)
 
hist(typeI_error, main="Significant nulls among selected", xlab="Count")
abline(v=0)
```
