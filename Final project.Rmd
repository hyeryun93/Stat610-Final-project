...
backward_stepwise <- function(dat) {
  ...
  while (TRUE) {
    ...
    for (j in 1:k) {
      ...
      rss_vec[j] <- sum(nw_fit$residuals^2)
    }
...
select_prob <- colMeans(sim_mat)
p1 <- results$p[results$term =="X1"]
p2 <- results$p[results$term == "X2"]
...
tapply(results$p, results$term, function(x) mean(x < 0.05, na.rm = TRUE))  
